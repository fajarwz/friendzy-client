                        ...
                        <form @submit.prevent="acceptFriend">
                            <green-button type="submit">
                                ...
                                <svg
                                    ...
                                    data-prefix="fas"
                                    data-icon="user-plus"
                                    class="fill-current w-4 h-4 ml-1 text-white svg-inline--fa fa-user-plus fa-w-20"
                        ...
                        <form @submit.prevent="denyFriend" class="ml-3">
                            <red-button type="submit">
                                ...
                                <svg
                                    ...
                                    class="fill-current w-4 h-4 ml-1 text-white svg-inline--fa fa-user-minus fa-w-20"
                                    xmlns="http://www.w3.org/2000/svg"
                                    viewBox="0 0 640 512"
                                >
                                    <path
...
export default {
    ...
    methods: {
        async getUser() {
            await this.$axios
                ...
                .then((response) => {
                    ...
                    this.isFriendsWith = response.isFriendsWith;
                    this.friendRequestSentTo = response.friendRequestSentTo;
                    this.friendRequestSentFrom = response.friendRequestSentFrom;
                })
                .catch((error) => {
                    console.log(error);
                });
        ...
        async acceptFriend() {
            await this.$axios
                .$put(`/api/friends/${this.$route.params.id}`)
                .then((response) => {
                    this.getUser();
                })
                .catch((error) => {
        ...
        async deleteFriend() {
            await this.$axios.$delete('/api/friends/'+this.$route.params.id)
